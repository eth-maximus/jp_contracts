...
contract JPToken is ERC20 {
    ...
    /*
        The PositionState is the status of the Position, whether it is Default (held on the JPToken)
    ...
    */
    ...
    /**
     * Throws if the sender is not the JPToken's manager
     */
    ...
    /**
     * Throws if JPToken is locked and called by any account other than the locker.
     */
    ...
    // List of initialized Modules; Modules extend the functionality of JPTokens
    ...
    mapping(address => IJPToken.ModuleState) public moduleStates;
    ...
    mapping(address => IJPToken.ComponentPosition) private componentPositions;
    ...
    /**
     * When a new JPToken is created, initializes Positions in default state and adds modules into pending state.
     * All parameter validations are on the JPTokenCreator contract. Validations are performed already on the 
     * JPTokenCreator. Initiates the positionMultiplier as 1e18 (no adjustments).
     *
        ...
        ERC20(_name, _symbol)
    {
        ...
        for (uint256 i = 0; i < _modules.length; i++) {
            moduleStates[_modules[i]] = IJPToken.ModuleState.PENDING;
        }
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. When a JPToken is locked, only the locker can call privileged functions.
     */
    ...
    /**
     * PRIVELEGED MODULE FUNCTION. Unlocks the JPToken and clears the locker
     */
    ...
    function addModule(address _module) external onlyManager {
        require(moduleStates[_module] == IJPToken.ModuleState.NONE, "Module must not be added");
        require(controller.isModule(_module), "Must be enabled on Controller");

        moduleStates[_module] = IJPToken.ModuleState.PENDING;

        emit ModuleAdded(_module);
    }

    /**
     * MANAGER ONLY. Removes a module from the JPToken. JPToken calls removeModule on module itself to confirm
    ...
    function removeModule(address _module) external onlyManager {
        ...
        require(moduleStates[_module] == IJPToken.ModuleState.INITIALIZED, "Module must be added");

        IModule(_module).removeModule();

        moduleStates[_module] = IJPToken.ModuleState.NONE;
    ...
    /**
     * MANAGER ONLY. Removes a pending module from the JPToken.
     */
    function removePendingModule(address _module) external onlyManager {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[_module] == IJPToken.ModuleState.PENDING, "Module must be pending");
    ...
    function initializeModule() external {
        require(!isLocked, "Only when unlocked");
        require(moduleStates[msg.sender] == IJPToken.ModuleState.PENDING, "Module must be pending");
        
        moduleStates[msg.sender] = IJPToken.ModuleState.INITIALIZED;
    ...
    /**
     * MANAGER ONLY. Changes manager; We allow null addresses in case the manager wishes to wind down the JPToken.
    ...
    function isInitializedModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IJPToken.ModuleState.INITIALIZED;
    }
    ...
    function isPendingModule(address _module) external view returns (bool) {
        return moduleStates[_module] == IJPToken.ModuleState.PENDING;
    }
     ...
     */
    function getPositions() external view returns (IJPToken.Position[] memory) {
        IJPToken.Position[] memory positions = new IJPToken.Position[](_getPositionCount());
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            if (_defaultPositionVirtualUnit(component) > 0) {
                positions[positionCount] = IJPToken.Position({
                    component: component,
                    module: address(0),
     ...
     *
     * Module must be initialized on the JPToken and enabled by the controller
     */
