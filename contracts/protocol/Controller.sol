...
contract Controller is Ownable {
    ...
    event JPAdded(address indexed _jpToken, address indexed _factory);
    event JPRemoved(address indexed _jpToken);
    ...
    // List of enabled JPs
    address[] public jps;
    // List of enabled factories of JPTokens
    address[] public factories;
    // List of enabled Modules; Modules extend the functionality of JPTokens
    address[] public modules;
    // List of enabled Resources; Resources provide data, functionality, or
    // permissions that can be drawn upon from Module, JPTokens or factories
    address[] public resources;

    // Mappings to check whether address is valid JP, Factory, Module or Resource
    mapping(address => bool) public isJP;
    ...
    /**
     * PRIVILEGED FACTORY FUNCTION. Adds a newly deployed JPToken as an enabled JPToken.
     *
     * @param _jpToken               Address of the JPToken contract to add
     */
    function addJP(address _jpToken) external onlyInitialized onlyFactory {
        require(!isJP[_jpToken], "JP already exists");

        isJP[_jpToken] = true;

        jps.push(_jpToken);

        emit JPAdded(_jpToken, msg.sender);
    }

    /**
     * PRIVILEGED GOVERNANCE FUNCTION. Allows governance to remove a JP
     *
     * @param _jpToken               Address of the JPToken contract to remove
     */
    function removeJP(address _jpToken) external onlyInitialized onlyOwner {
        require(isJP[_jpToken], "JP does not exist");

        jps = jps.remove(_jpToken);

        isJP[_jpToken] = false;

        emit JPRemoved(_jpToken);
    }
    ...
    function getJPs() external view returns (address[] memory) {
        return jps;
    }

    /**
     * Check if a contract address is a module, JP, resource, factory or controller
     *
     * @param  _contractAddress           The contract address to check
     */
    function isSystemContract(address _contractAddress) external view returns (bool) {
        return (
            isJP[_contractAddress] ||
