...
library Invoke {
     ...
     *
     * @param _jpToken        CKToken instance to invoke
        ...
        internal
    {
        ...
        _jpToken.invoke(_token, 0, callData);
    }
        ...
        internal
    {
        if (_quantity > 0) {
            ...
            uint256 existingBalance = IERC20(_token).balanceOf(address(_jpToken));

            Invoke.invokeTransfer(_jpToken, _token, _to, _quantity);

            // Get new balance of transferred token for CKToken
            uint256 newBalance = IERC20(_token).balanceOf(address(_jpToken));
     ...
     */
    function invokeUnwrapWETH(IJPToken _jpToken, address _weth, uint256 _quantity) internal {
        bytes memory callData = abi.encodeWithSignature("withdraw(uint256)", _quantity);
        _jpToken.invoke(_weth, 0, callData);
    }
     ...
     * @param _quantity        The quantity to unwrap
     */
    function invokeWrapWETH(IJPToken _jpToken, address _weth, uint256 _quantity) internal {
        bytes memory callData = abi.encodeWithSignature("deposit()");
        _jpToken.invoke(_weth, _quantity, callData);
    }
}